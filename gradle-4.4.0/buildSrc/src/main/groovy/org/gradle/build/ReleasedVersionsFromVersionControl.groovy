...
class ReleasedVersionsFromVersionControl implements ReleasedVersions {
    ...
    ReleasedVersionsFromVersionControl(File versionsFile) {
        ...
        def latestNonFinalRelease = [mostRecentSnapshot, mostRecentRc].findAll {
            it.version > latestFinalRelease.version
        }.sort { it.buildTimeStamp }.reverse().find()
        allVersions = ([latestNonFinalRelease] + finalReleases).findAll().findAll {
